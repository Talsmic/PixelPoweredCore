...
_drawText(box_x+4,box_y+2,"Ideal Sizes:",c_orange,1);

//Draw Table Contents
_setAlign(fa_right);
draw_array_vertical(box_x+ui_ListBackground_get_width()*0.5-4,box_y+13,col1,c_gray9,1,12);
if ( col1[0] = window_get_width() ) { _drawText(box_x+ui_ListBackground_get_width()*0.5-4,box_y+13,col1[0],c_shadow_light,1) };
for ( var i=1; i<array_length_1d(col1); i++ ) {
	if ( col1[i] = window_get_width() ) { _drawText(box_x+ui_ListBackground_get_width()*0.5-4,box_y+13+12*i,col1[i],c_shadow_light,1) };
...
if ( col2[0] = window_get_width() ) { _drawText(box_x+ui_ListBackground_get_width()*0.5+4,box_y+13,col2[0],c_shadow_light,1) };
for ( var i=1; i<array_length_1d(col2); i++ ) {
	if ( col2[i] = window_get_height() ) { _drawText(box_x+ui_ListBackground_get_width()*0.5+4,box_y+13+12*i,col2[i],c_shadow_light,1) };
