...
if ( col1[0] = _windowWidth() ) { drawText(box_x+ui_ListBackground_get_width()*0.5-4,box_y+13,col1[0],c_shadow_light,1) };
for ( var i=1; i<array_length_1d(col1); ++i ) {
	if ( col1[i] = _windowWidth() ) { drawText(box_x+ui_ListBackground_get_width()*0.5-4,box_y+13+12*i,col1[i],c_shadow_light,1) };
	if ( col1[i-1] < _windowWidth() and col1[i] > _windowWidth() ) {
...
if ( col2[0] = _windowWidth() ) { drawText(box_x+ui_ListBackground_get_width()*0.5+4,box_y+13,col2[0],c_shadow_light,1) };
for ( var i=1; i<array_length_1d(col2); ++i ) {
	if ( col2[i] = _windowHeight() ) { drawText(box_x+ui_ListBackground_get_width()*0.5+4,box_y+13+12*i,col2[i],c_shadow_light,1) };
	if ( col2[i-1] < _windowHeight() and col2[i] > _windowHeight() ) {
