///_arrayCompare_Greaters(arrayA,arrayB,[compare_empties],[required_quantity],[required_quality]);
/// @arg arrayA					{array}		
/// @arg arrayB					{array}	
/// @arg [compare_empties]		{boolean}	(default: true)
/// @arg [required_quantity]	{integer}	
/// @arg [required_quality]		{real}		
/*
	<< Returns >>	{boolean}
*/ 
...
if ( !_arrayA_2d or !_arrayB_2d ) {
	...
	for ( var i=0 ; i<_array_length ; ++i ) {
		...
		if ( i < _array_lengthA and i < _array_lengthB ) {
			if ( compare_quality )	{ if ( _arrayA[i] >= _arrayB[i] + required_quantity )	{ if ( compare_quantity ) { output++ } else { return true } } }
			else					{ if ( _arrayA[i] > _arrayB[i] )						{ if ( compare_quantity ) { output++ } else { return true }	} };
			}
		//If you have no second value, compare against zero
		else if ( _compare_empties and i < _array_lengthA ) {
			if ( compare_quality )  { if ( _arrayA[i] >= 0 + required_quantity )			{ if ( compare_quantity ) { output++ } else { return true } } }
			else					{ if ( _arrayA[i] > 0 )									{ if ( compare_quantity ) { output++ } else { return true }	} }
			}
		else if ( _compare_empties and i < _array_lengthB ) {
			if ( compare_quality )  { if ( _arrayB[i] <= 0 + required_quantity )			{ if ( compare_quantity ) { output++ } else { return true } } }
			else					{ if ( _arrayB[i] < 0 )									{ if ( compare_quantity ) { output++ } else { return true }	} }
			};
...
//2D Comparison
	...
	for ( var j=0 ; j<_array_height ; ++j ) {
		...
		for ( var i=0 ; i<_array_length ; ++i ) {
			...
			//If you have two values, compare them
			if ( i < _array_lengthA and i < _array_lengthB ) {
				if ( compare_quality )	{ if ( _arrayA[j, i] >= _arrayB[j, i] + required_quantity )	{ if ( compare_quantity ) { output++ } else { return true } } }
				else					{ if ( _arrayA[j, i] > _arrayB[j, i] )						{ if ( compare_quantity ) { output++ } else { return true }	} };
				}
			//If you have no second value, compare against zero
			else if ( _compare_empties and i < _array_lengthA ) {
				if ( compare_quality )  { if ( _arrayA[j, i] >= 0 + required_quantity )				{ if ( compare_quantity ) { output++ } else { return true } } }
				else					{ if ( _arrayA[j, i] > 0 )									{ if ( compare_quantity ) { output++ } else { return true }	} }
				};
			else if ( _compare_empties and i < _array_lengthB ) {
				if ( compare_quality )  { if ( _arrayB[j, i] <= 0 + required_quantity )				{ if ( compare_quantity ) { output++ } else { return true } } }
				else					{ if ( _arrayB[j, i] < 0 )									{ if ( compare_quantity ) { output++ } else { return true }	} }
				};
