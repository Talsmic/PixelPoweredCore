///_drawSprite_Part_Tiled([x],[y],[sprite_id],[image_id],[colour],[alpha],[clip_region],[tile_x],[tiles_y]);
...
var _tiles_x =	argument_count > 7 ? argument[7] : 1;
var _tiles_y =	argument_count > 8 ? argument[8] : 1;
#endregion

for ( var i=1; i<=_tiles_y; i++ ) {
	for ( var j=1; j<=_tiles_x; j++ ) {
		draw_sprite_general( _sprite, _image, _clip[eR.x1], _clip[eR.y1], _clip[eR.w], _clip[eR.h], _x, _y, 1, 1, 0, _colour[0], _colour[1], _colour[2], _colour[3], _alpha );
		_x += _clip[eR.w];
		};
