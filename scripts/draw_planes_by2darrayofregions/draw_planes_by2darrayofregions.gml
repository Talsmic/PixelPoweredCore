///draw_planes_by2darrayofregions({array_of_regions},[outline]);
...
for ( var i=0; i<array_height_2d(array_of_regions); i++ ) {
	for ( var j=0; j<array_length_2d(array_of_regions,i); j++ ) {
		...
		if is_array(region_array) {
			draw_plane(region_array[eR.x1],region_array[eR.y1],region_array[eR.w],region_array[eR.h],find_colour(j),draw_get_alpha(),0,outline);
			};
		};
	};