///drawText_Outlined(x,y,string,[colour],[ol_colour],[alpha],[thickness],[shadowed_thickness],[shadow_alpha],[shadow_tint]);
/// @arg x				{real}
/// @arg y				{real}
/// @arg text			{string}
...
var thickness =		argument_count > 6 ? _arrayOfLength(argument[6], 4) : [1,1,1,1];
var thickness2 =	argument_count > 7 ? _arrayOfLength(argument[7], 4) : [0,0,0,0];
...
//Create Surface
var text_surface = surface_create( text_region[aR.w], text_region[aR.h] );
...
surface_set_target( text_surface ) {
	...
	var i, j;
	//Shaded Outline
	if ( _arrayCompare_Greaters(thickness2,thickness) ) {
		for ( i=-thick2A[0] ; i<=thick2A[2] ; ++i ) {
			for ( j=-thick2B[1] ; j<=thick2B[3] ; ++j ) {
				drawText(surfacedraw_x+i,surfacedraw_y+j,text,shadow_colour,1);
				};
			};
	...
	for ( i=-thickA[0] ; i<=thickA[2] ; ++i ) {
		for ( j=-thickB[1] ; j<=thickB[3] ; ++j ) {
			drawText(surfacedraw_x+i,surfacedraw_y+j,text,colour_ol,1);
			};
		};
	...
	//Normal Text
	drawText(surfacedraw_x,surfacedraw_y,text,colour,1);
